...
while survivor_path == None:
	# calculate nexts
	all_nexts = {}
	for (seq, (x, y)), p in survivors.items():
		nexts = [
			(x, y),
			(x+1, y),
			(x, y+1),
	...
	# prune collisions
	survivors = {}
	for (seq, n), path in all_nexts.items():			
		if n in grid:
			continue
		seq2 = seq
		if seq2 == 0 and p == exit_p:
			seq2 = 1
		elif seq == 1 and p == start_p:
			seq2 = 2
		survivors[(seq2, n)] = path
		if n == exit_p and seq2 == 2:
			print(f"surviving path to exit and then back to start and then back to exit found")
			survivor_path = path
