...
for i, (p,v) in enumerate(zip(ps, vs)):
	print(f"x + u {ts[i]} = {p[0]} {'-' if v[0] < 0 else '+'} {'' if abs(v[0]) == 1 else abs(v[0])} {ts[i]}")
	print(f"y + v {ts[i]} = {p[1]} {'-' if v[1] < 0 else '+'} {'' if abs(v[1]) == 1 else abs(v[1])} {ts[i]}")
	print(f"z + w {ts[i]} = {p[2]} {'-' if v[2] < 0 else '+'} {'' if abs(v[2]) == 1 else abs(v[2])} {ts[i]}")
	if i >= 2:
		break
