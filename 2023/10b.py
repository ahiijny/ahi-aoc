...
for i, cur in enumerate(path):
	...
	if delta != prev_delta:
		...
		# need to cover two previous as well if we did a left turn. right turns are closed in so we dont worry
		
		if left:
			#print("left turn")
			blot = to_add[-1]
			to_add.append((blot[0] - delta[0], blot[1] - delta[1]))
			to_add.append((blot[0] - 2 * delta[0], blot[1] - 2 * delta[1]))
