...
# do clockwise
# in a clockwise walk, build inner wall on right
# every time we do a turn, we seal the edge right behind us
# edges stored as (x1, y1), (x2, y2) where x1 < x2, y1 < y2
# more than one inner wall can be on the exact some position (e.g. in the case of a snake
...
for i, cur in enumerate(path):
	...
	prev_delta = (path[i-1][0] - path[i-2][0], path[i-1][1] - path[i-2][1]) # praise python negative indexes
	right = None
	if delta == (1, 0):
		right = (0, 1)
	elif delta == (0, 1):
