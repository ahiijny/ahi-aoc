...
end_cheats = []
max_length = 20 - 1 # -1 bc 1 second is used to enter starting cheat location. so e.g. S-E if S and E form an edge of length 1, takes 2 picoseconds to nagivate
...
for y in range(H):
	...
	for x in range(W):
		...
		for d in end_cheats:
			c2 = add(c1, d)
			cheat_length = abs(c2[1] - c1[1]) + abs(c2[0] - c1[0]) + 1
			if not bounds(*c2):
				continue
			if c2 not in best:
				continue
			if grid[c2[1]][c2[0]] == '#':
				continue
			# check all possible entrypoints
			...
			for entry_d in dirs:
				...
				if c0 not in best:
					continue
				cheat_improvement = best[c2] - best[c0] - cheat_length
