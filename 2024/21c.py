...
def opt(pad_idxs, code):
	...
	for c in code:
		...
		for r in routes:
			...
			if best_root_len is None or best_root_len > opt_seqs[-1]:
				best_root_len = opt_seqs[-1]
				best_seqs = opt_seqs
				best_route = next_code
		
		if super_seq is None:
			super_seq = [len(best_route)] + best_seqs
		else:
			super_seq[0] = super_seq[0] + len(best_route)
